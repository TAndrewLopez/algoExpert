const isValidSudoku = (board: string[][]): boolean => {
  // VERIFY COLS HAVE 1 - 9 WITH NO DUPLICATES
  // VERIFY ROWS HAVE 1 - 9 WITH NO DUPLICATES
  // VERIFY 3x3 HAVE 1 - 9 WITH NO DUPLICATES

  return true;
};

const returnRow = (board: string[][], row: number): string[] => {
  return board[row];
};

const returnCol = (board: string[][], col: number): string[] => {
  const sudokuCol: string[] = [];
...
const return3x3 = (board: string[][], x: number, y: number) => {
  const section = [];
  let row1, row2, row3;
  if (y === 0) {
...
// console.log(returnRow(sudokuPuzzle1, 0))
// console.log(returnCol(sudokuPuzzle1, 1))
